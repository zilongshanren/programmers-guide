...
`Sprite3D` currently supports 3 file formats

* `.obj` files

both 3d Studio Max and Maya supports exporting `.obj` format. It is important to
understand that the `.obj` format don't support animations.

* `c3t`, `c3b` files

both file formats are developed by Chukong, `.c3t` is text format and you can
use it to debug your model. `.c3b` is binary format that optimized for production
use. You generate them using `fbx-conv` tool we'll cover that later in this chapter.
...
be created with the `.c3b` or `.c3t` files. And then create the `Animate3D` through
the `Animation3D` object.
```cpp
...
length` parameters you can create multiple animations. The unit for both parameters is seconds. Example:
```cpp
...
the `src` indicates clipping plane distance information, -1 means the near clipping
