...
`Sprite3D` currently supports 2 file formats

* [Wavefront Object](http://en.wikipedia.org/wiki/Wavefront_.obj_file) files: `.obj` files
* Cocos2d-x 3d ad-hoc format:`c3t`, `c3b` files.

Cocos2d-x supports Wavefront file format because it is widely adopted by the 3D
editors, and it is extremely easy to parse. But it is limited, and doesn't support
advanced features like animations.

On the other hand, `c3t` and `c3b` are Cocos2d-x own file format that were created
after to allow animations, materials and other advanced 3d features.
The suffix `t` means Text, while the suffix `b` means binary. Users must use
`c3b` for production because it is more efficient. In case users want to debug the
file format, and track its changes in git or any other VCS, they can use `c3t` instead.

Cocos2d-x provides a tool called `fbx-conv` that allows the conversion of FBX file
formats into `c3b` or `c3t`. FBX is the most popular 3D file format and is being
supported by all the major editors like: Cinema4D, Maya, [Blender](link to blender),
XXX, XXX, XXX

XXX: Add a section in this chapter regarding how to use fbx-conv

## Working with Sprite3D

### Attach 3D Model

XXX: Explain in more detail this section. How to define the attaching nodes ?
XXX: In Maya? By hand ? Show example

You can add 3D models to other 3D models to create rich effects. An example would
be adding a weapon to a character. For this use `AttachNode(name)` function with
the name of the node you want it to attach to.
```cpp
...
XXX: similar to previous section. More info.
XXX: Detail what is a Model. Why a model has meshes ? What is a mesh ?

If the 3D model is comprised from _meshes_ you can access the
...
for a character. Lets take a look at an example of a character wearing a coat:

![](9/9_4.png)

We can change the coat that she is wearing by changing the visibility of _mesh_.
...
## Creating Assets
To use `fbx-conv` to create c3b and c3t files for sprite3D, you need to export
your 3d assets into `.fbx` format first. It is simple to use:
...
* fbx-conv exports c3b by default

* The model needs to have a material that contain at least one texture

* `fbx-conv` only supports skeletal animation.

* `fbx-conv` only supports one skeleton object no multiple skeleton support yet.
...
## Animation
To run an 3d animation, you can use `Animation3D\Animate3D`. `Animation3D` can
be created with the `.c3b` or `.c3t` files. And then create the `Animate3D`
through the `Animation3D` object.
```cpp
...
```
### Animation Blending
Cocos2d-x will automatically apply blending to animation between two animations,
...
smaller effect, _orthogonal camera_ to see objects as large distance.  The
`Camera` inherits from `Node`, can support most `Action` objects.
...
Just use `setCameraMask(CameraFlag)` on the node and set camera flag
`setCameraFlag(CameraFlag)` on the camera.
```cpp
//Camera
...
## Light
Light is really important for building mood and ambiance for a game. Cocos2d-x
supports 4 types of lights:
* Ambient Light
* Directional Light
* Point Light
* Spot Light.
