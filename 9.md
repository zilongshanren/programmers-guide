...
```
This creates a `Sprite3D` object based upon the `.c3b` file:
![](9/9_1.png)

`Sprite3D` currently support 3 file formats

* [`.obj`](http://people.sc.fsu.edu/~jburkardt/data/obj/obj.html) files

 both 3ds Max and Maya supports exporting `.obj` format.
 >Note: .obj format don't support animation

* `c3t`, `c3b` files

 both file formats are developed by chukong, `.c3t` is text format and you can use it to debug your model. `.c3b` is binary format that optimized for production use. you can generate them using `fbx-conv` tool we'll cover that later

## Play with Sprite3D

### Attach 3D Model
You can add 3D models to other 3D models to create rich effects. An example would
be adding a weapon to a character. For this use `AttachNode(name)` function with the name of the node you want it to attach to.
```cpp
...
### Swap 3D Model
If the 3D model is comprised from many pieces of _meshes_ you can access the
_mesh data_ using `getMeshByIndex()` and `getMeshByName()`. Using these
functions it is possible to achieve something like swap weapon or clothing for a character. Lets take a look at an example:

Following we have a character wearing a coat

![](9/9_4.png)

We can change the coat that she is wearing by changing the visibility of _mesh_. The following example demonstrates how to do this:
```cpp
...
## Creating Asset
To use `fbx-conv` to create c3b and c3t files for sprite3D, you need to export your 3d assets into `.fbx` format first.
...
> Note: fbx-conv exports c3b by default

>model need to have a material that contain at least one texture

>only supports skeletal animation.

>only supports one skeleton object no multiple skeleton support yet.

> You can create a 3d scene by exporting multiple static model

> The maximum amount of vertices or indices a mesh are 32767


## Animation
To run an 3d animation, you can use `Animation3D\Animate3D`. `Animation3D` can be created with 'c3b' or 'c3t' files. And then create the `Animate3D` through the Animation3D object. .
```cpp
...
### Multiple animation
Using animation start time, animation
length parameter you can create multiple animations, for example:
```cpp
...
```
>Note: the unit for both parameters are in seconds

### Animation Blending
Cocos2d-x will automatically apply blending to animation between two animations, the default transition time is 0.1
seconds , you can set the transition time by `Animate3D::setTransitionTime`.

>Tip: Cocos2d-x only supports linear interpolation between keyframes.
...
### Camera Use
Each `Scene` automatically creates a default camera, based on the projection of `Director`. If you need more than one camera, you can use
...
Hiding object from one camera is very easy.
Just use `setCameraMask(CameraFlag)` on the node and set camera flag `setCameraFlag(CameraFlag)` on the camera.
```cpp
//Camera
...
`Camera` provide helpers to transform coordinates from screen space to world space
```cpp
void unproject (const Size & viewport, Vec3 * src, Vec3 * dst) const;
```
Which viewport is viewport size, use src as screen coordinates, the z axis of the src indicates
clipping plane distance information, -1 means the near clipping plane, 1 means
far clipping plane. dst will return world space coordinates.


## Light
Light is really important for building mood and ambiance for a game. cocos2d-x supports 4 types of lights
* Ambient Light
* Directional Light
* Point Light
* Spot Light.
...
Ambient Light will aplly for everything in the scene.
You can create ambient light with the following code:
```cpp
...
Directional Light often used to simulate a light source such as sunlight, you can create a directional light with the code below:
```cpp
...
Point Light often used to simulate the effect of light bulbs or torch you can create a point light source with the code below:
```cpp
...
Spot Light often used to simulate flashlight. you can create a spotlight and with the following code:
```cpp
...
## Ray
Ray is super useful in 3D game. You can use Ray to achieve things like picking up a object in 3D, or detect collision in 3D.
Illustration:
...
You need two vectors to create a Ray, one is that the origin, the other is the direction
...
_GLProgram state set_, _texture_, _bone_, _blending equations_, _AABB_ and any other
