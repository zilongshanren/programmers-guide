...
```cpp
auto button = Button::create("normal_image.png", "selected_image.png", "disabled_image.png", ui::Widget::TextureResType::PLIST);
...
possible states the button can be in. A `Button` is made up of 3 graphics that might look like this in it's various states:
...
```
As you can see in the above example we specify a _png_ image for each of the
possible states the checkbox can be in. A `CheckBox` is made up of 5 graphics that might look like this in it's various states:
...
slider->loadBarTexture("sliderTrack.png"); // what the slider looks like
...
might look like this in it's various states:
