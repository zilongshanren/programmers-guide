...
Cocos2d-x provides `Scene`, `Transitions`, `Sprite`, `Menu`, `Sprite3D`, `Audio` objects and much more. Everything you need to create your games is included.
...
Building on this concept, we can think of a `Scene` as a collection of `Node` objects. Lets break the scene above down to see the the *scene graph* uses the *z-order* to layout out the `Scene`:
