...
Cocos2d-x is a cross-platform game engine. What is a game engine? Don't get scared just yet! A game engine is a piece of software that provides common functionality that all games might have. You might hear this definition refered to as an API or framework. For this guide we are going to just use the term game engine. 
...
## Main Components
It might seem overwhelming at first, but getting started with Cocos2d-x is simple. Before we dive into depth we must understand some of the concepts Cocos2d-x utilizes. At the heart of Cocos2d-x are `Scenes`, `Nodes`, `Sprites`, `Menus` and `Actions`. Look at any of your favorite games and you will see all of these components!
...
This is a main menu and it is a single `Scene`. Yes, this scene is made up of several pieces that all fit together to give us the end result. Scenes are drawn by the renderer. The renderer generates the 2d or 3d images we are using from a model. This can include texture, lighting and shading. To better understand this we need to talk a bit about a `Scene Graph`. No, its not something you will find in a math textbook.
...
Another point to think about is elements with a  negative `z-order` are on the left side of the tree while elements with a positive `z-order` are on the right side of the tree. Keep this in consideration when ordering your elements!

Building on this concept, we can think of a `Scene` as a collection of `Nodes`. Lets break the scene above down to see the the `scene graph` uses the `z-order` to layout out the `Scene`:
...
Lastly, all `Nodes` (remember that a `Sprite` is a subclass of `Node`) have a value for `anchor point`. We haven't talked about this yet, so now is a good time. You can think of `anchor point` as a way of specifying what part of the sprite will be used when setting the position of it.
...
## Actions
Creating a `Scene` and adding `Sprites` on the screen is only part of what we need to do. For a game to be a game we need to make things move around! `Actions` are an integral part of every game. `Actions` allow the transformation of `Nodes` in time space. Want to move a `Sprite` from one `Point` to another and use a callback when complete? No problem! You can even create a `Sequence` of `Actions` to be performed on a `Node`. You can change `Node` properties like position, rotation and scale. Example Actions: `MoveBy`, `Rotate`, `Scale`. All games use `Actions`.
...
myNode->runAction(Spawn::create(moveTo1, moveBy1, moveTo2, NULL));
```
...
## Conclusion
We have gone throught a lot of Cocos2d-x concepts. Take a deep breath. Dont worry. Just dive in with your ideas and take it one step at a time. Cocos2d-x and progamming in general are not a skill that is learned overnight. These take practice and understanding. Remember that the forums are also there to help you with questions.
