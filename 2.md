...
This example runs a `Sequence`, in order, but what about running all the specified `Actions` all at the same time? Cocos2d-x supports this too and it is called `Spawn`. `Spawn` will take all the specified `Actions` and execute them all at the same time. Some might be longer than others so they wont all finish at the same time if this is the case.
