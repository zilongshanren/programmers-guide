...
FIXME: Kind of confusing. I would say that `By` is relative, and `To` is absolute.

FIXME: Add specific examples

FIXME: Better introduction to `By` and `To`.

FIXME: Screenshots needs to use default anchor point of 0.5, 0.5
...
and the action performs incrementally based upon the `Node` objects current properties.
...
// fades in the sprite in 5 seconds
...
to keep in mind is that regardless of speed, ease actions always start and finish at
the same time.

Easing functions displayed over a graph.

FIXME: Explain in more detail the use case. Why should I use them ? When ?

FIXME: I don't think we can use this graph.
...
`Spawn` produces the same result as running multiple consecutive `runAction()` statements.
However, the benefit of spawn is that you can put it in a `Sequence` to help achieve
specific effects that you cannot otherwise. Combining `Spawn` and `Sequence`
is a very powerful feature

FIXME: Perhaps splitting the sample in 2 ?
Example:
```cpp
...
FIXME: Add a flow chart for this
...
FIXME: Add detailed example. Create a Sequence, and then reverse it and explain how it works.
