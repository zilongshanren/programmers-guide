...
## 3D Sprite
### 介绍
Sprite3D负责对三维模型进行管理，包括模型网格，骨骼数据，渲染数据等。可通过路径载入三维模型，支持的模型格式包括.c3t .c3b .obj三种。Sprite3D派生自Node，继承了Node的大部分功能。虽然该类针对三维模型，但由于二维和三维的相似性，很多的功能特性和Sprite非常类似，同样支持Action。
### Sprite3D的使用
如何载入三维模型并显示？可通过在自定义Layer中的onEnter方法或init方法或构造函数中添加如下代码来载入三维模型并进行渲染：

	auto size = Director::getInstance()->getWinSize();
    std::string fileName = "Sprite3DTest/boss1.obj";
...
如何开启模型的骨骼动画？可以通过Animation3D类读取和模型同名的文件来创建骨骼动画，关于Animation3D的具体使用可参看对应部分的说明文档，在此只说明如何通过该类创建动画。创建了Sprite3D后可通过如下代码开启模型对应的骨骼动画：
...
![](./10/SkinModel1.png)  ![](./10/SkinModel2.png)

如何给模型添加附加模型(例如武器)？有时需要给某一个模型附加另外的一个模型用以表现丰富的效果，这样就需要使用到Sprite3D的AttachNode，AttachNode和模型的骨骼相关，在载入模型的时候已经构建好了对应的AttachNode列表，使用的时候只需要通过名称获得对应的AttachNode并把需要附加的模型挂在AttachNode即可实现，例如把武器绑定在怪物模型的右手上的代码如下：
...
![](./10/AttachModel1.png)  ![](./10/AttachModel2.png)

有些情况下模型可能由多个网格数据组成，如何获取模型的网格数据？Sprite3D提供了对应的函数来获取该数据：

    Mesh* getMeshByIndex(int index) const;
    Mesh* getMeshByName(const std::string& name) const;

我们可以通过索引或者名称的方式索引网格数据，并对该数据进行处理。例如给人物模型进行换装，每一件衣物都是由对应的网格数据组成，我们就需要设置当前希望显示的衣物网格处于Visible状态,而设置其他网格处于隐藏状态，部分代码如下所示：
...
其次，显示第一套上衣：
    ...
    auto girlPants0 = sprite->getMeshByName("Girl_UpperBody01");
    if(girlPants0){
        girlPants0->setVisible(true);
    }
	auto girlPants1 = sprite->getMeshByName("Girl_UpperBody02");
	if(girlPants1){
		girlPants1->setVisible(false);
	}
...
然后，显示第二套上衣：
    ...
    auto girlPants0 = sprite->getMeshByName("Girl_UpperBody01");
    if(girlPants0){
        girlPants0->setVisible(false);
    }
	auto girlPants1 = sprite->getMeshByName("Girl_UpperBody02");
	if(girlPants1){
		girlPants1->setVisible(true);
	}

**完整代码请参看CppTest的Sprite3D例子中的Sprite3DReskinTest**

效果如下图所示：

![](./10/Reskin1.png)  ![](./10/Reskin2.png)
...
Cocos2d-x不支持自定义关键帧之间的插值。如果你使用一个不同于线性插值,fbx-conv将产生额外的关键帧来补偿。这种补偿是根据目标帧完成。尽管FBX目标FPS是可定制的,不是所有的建模应用程序允许您更改这个值(默认是30 FPS)。为了避免腹胀文件与额外的关键帧,一定要尽量使用线性插值。
### 如何创建使用
...
### 如何使用切分动画
通过传入动画数据、动画开始时间(单位秒)、动画周期参数(单位秒)可以把动画数据切分为多个动画使用，如下所示:
...
![](./10/VIEW_POINT_ORIENTED.png) ![](./10/VIEW_PLANE_ORIENTED.png)
