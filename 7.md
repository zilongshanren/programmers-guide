...
#### Feature List
1. Touch event capturing.
todo
2. Clipping: stencil VS scissor.
todo
3. UTF8 enabled text display and text input.
todo
4. Percent position and percent content size for layout.
todo
...
#### Limitation
1. Currently the RichText widget doesn't support clickable item, and the layout implementation is also very poor.
2. The TextField widget may not be integrated into the system UI, you might want to use EditBox at the moment.
3. ListView doesn't support DataSource and also doesn't support item reuse.
