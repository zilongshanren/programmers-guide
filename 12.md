...
Image that we have an sprite and we want it to fall down at a certain speed
(due to gravity). Then it touches ground (collision detection) and then it bounces
back (elasticy, friction). How can we do that ? Briefly explain how to do manually,
and show that it is complex and that it will not scale (what about if you have
  100 sprites). Then tell that a physics engine solves that for us, and show to
  do that simple example using the cocos2d-x API.

## What options and why integrated physics engine
Simulating a real physics world in a game can be a daunting task. Usually a
_physics engine_ can help make this easier. Cocos2d-x provides a _physics engine_
that is based on the popular [Chipmunk](https://chipmunk-physics.net) physics
engine.
